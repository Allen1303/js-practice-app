...
export default function App() {
  ...
  const handleLogout = async () => {
    if (
      confirm(
        "Are you sure you want to sign out from Cloud Sync? Your progress remains secure in local storage.",
      )
    ) {
      try {
        await logoutUser();
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        setIsSyncing(true);
        try {
          const firestoreData = await getUserProgressFromFirestore(
            currentUser.uid,
          );

          const localCodesRaw = localStorage.getItem("learnjs_play_codes");
          const localProgressRaw = localStorage.getItem(
              ...
              <div className="w-full md:w-1/2 border-b md:border-b-0 md:border-r border-zinc-200 flex flex-col bg-[#faf8f5] overflow-y-auto min-h-0 min-w-0">
                ...
                <div className="flex border-b border-zinc-200 bg-zinc-50 shrink-0 select-none overflow-x-auto justify-start">
                  <button
                    ...
                    className={`py-3 px-4 text-[12px] font-bold tracking-tight flex items-center justify-center gap-1.5 transition-all cursor-pointer relative font-sans ${
                      leftTab === "problem"
