...
import { KNOWLEDGE_MAP_CATEGORIES, KNOWLEDGE_MAP_TOPICS } from "./data/knowledgeMap.js";
import { runExerciseTests, runCustomEvaluation } from "./utils/executor.js";
import { getSampleUsage, getTopDeclarations, getBottomUsage, getFullExerciseTemplate } from "./utils/sampleGenerator.js";
import {
  auth,
  signInWithGoogle,
  logoutUser,
...
import { CheckCircle, Code, BookOpen, Sparkles, Edit3, RotateCcw, Globe } from "lucide-react";
...
export default function App() {
  ...
  // Notebook interactive notes mapping
    ...
    try {
      ...
      if (storedCodes) {
        ...
        Object.keys(parsed).forEach(key => {
          const exercise = CONCEPTS.flatMap(c => c.exercises).find(e => e.id === key);
          if (exercise) {
            const originalTemplate = exercise.codeTemplate;
            const parsedVal = parsed[key];
            let nextVal = parsedVal;
            if (key === "string-template-interpolation" && typeof nextVal === "string") {
              // Replace old stale expected comments to prevent user confusion
              nextVal = nextVal.replace(/Welcome,\s*Alice!\s*Your\s*role\s*is:\s*admin\./gi, 'Welcome Alice, your role is admin');
              nextVal = nextVal.replace(/Welcome,\s*Bob!\s*Your\s*role\s*is:\s*moderator\./gi, 'Welcome Bob, your role is moderator');
              // Correct the colon insertion so active code matches official assert expectations
              nextVal = nextVal.replace(/your\s*role\s*is:\s*\$\{role\}/gi, 'your role is ${role}');
            }

            const cleanParsed = typeof parsedVal === "string" ? parsedVal.split("// Sample usage")[0].trim() : "";
            const cleanOriginal = typeof originalTemplate === "string" ? originalTemplate.trim() : "";
            const cleanTopOriginal = (getTopDeclarations(exercise) + originalTemplate).trim();

            if (typeof nextVal === "string") {
              const funcBody = nextVal.split("// Sample usage")[0].trim();
              const topDecs = getTopDeclarations(exercise);

              if (cleanParsed === cleanOriginal || cleanParsed === cleanTopOriginal || !parsedVal || parsedVal === originalTemplate) {
                ...
                const bodyStartIdx = funcBody.search(/\b(function|async\s+function|class)\b/);
                if (bodyStartIdx !== -1) {
        ...
        try {
          ...
          console.warn("Could not save migrated codes on initialization: ", storageErr);
        }
      } else {
      ...
      const storedNotes = localStorage.getItem("learnjs_play_notes");
      if (storedNotes) {
  ...
  // Handle Cloud Sign In and Session Load
  ...
  const handleLogout = async () => {
    if (confirm("Are you sure you want to sign out from Cloud Sync? Your progress remains secure in local storage.")) {
      try {
        await logoutUser();
        setUser(null);
      } catch (err) {
  ...
  // Keep Firestore in Sync on user transitions
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        setIsSyncing(true);
        try {
          const firestoreData = await getUserProgressFromFirestore(currentUser.uid);

          const localCodesRaw = localStorage.getItem("learnjs_play_codes");
          const localProgressRaw = localStorage.getItem("learnjs_play_progress");
          const localTopicsRaw = localStorage.getItem("learnjs_covered_topics");
          const localNotesRaw = localStorage.getItem("learnjs_play_notes");

          const localCodes = localCodesRaw ? JSON.parse(localCodesRaw) : {};
          const localProgress = localProgressRaw ? JSON.parse(localProgressRaw) : {};
          const localTopics = localTopicsRaw ? JSON.parse(localTopicsRaw) : {};
          const localNotes = localNotesRaw ? JSON.parse(localNotesRaw) : {};

          if (firestoreData) {
            const mergedCodes = { ...firestoreData.codes, ...localCodes };
            const mergedProgress = { ...firestoreData.progress, ...localProgress };
            const mergedTopics = { ...firestoreData.completedTopics, ...localTopics };
            const mergedNotes = { ...firestoreData.notes, ...localNotes };

            setUserCodes(mergedCodes);
            setSolvedExercises(mergedProgress);
            setCompletedTopics(mergedTopics);
            setUserNotes(mergedNotes);

            localStorage.setItem("learnjs_play_codes", JSON.stringify(mergedCodes));
            localStorage.setItem("learnjs_play_progress", JSON.stringify(mergedProgress));
            localStorage.setItem("learnjs_covered_topics", JSON.stringify(mergedTopics));
            localStorage.setItem("learnjs_play_notes", JSON.stringify(mergedNotes));

            await saveUserProgressToFirestore(currentUser.uid, {
              codes: mergedCodes,
  ...
  const toggleTopicProgress = (topicId) => {
    ...
    try {
      ...
      if (auth.currentUser) {
        saveUserProgressToFirestore(auth.currentUser.uid, {
          codes: userCodes,
          progress: solvedExercises,
    ...
    if (confirm("⚠️ CRITICAL RESET ⚠️\n\nThis action will completely delete all of your:\n- Solved progress indicators\n- Custom modified solutions\n- Checklist markers on the Roadmap\n- Interactive Notebook notes\n\nAre you sure you want to delete everything and start fresh?")) {
      if (confirm("Confirm one final time: This cannot be undone. Are you absolutely sure?")) {
        try {
          ...
          alert("Reset successful. All states restored to pristine default conditions.");
  ...
  // Save changes to localStorage and Firestore on state changes
  ...
  const saveProgressToStorage = (updatedProgress) => {
    ...
    try {
      localStorage.setItem("learnjs_play_progress", JSON.stringify(updatedProgress));
      if (auth.currentUser) {
        saveUserProgressToFirestore(auth.currentUser.uid, {
          codes: userCodes,
  ...
  const activeExercise = activeConcept.exercises[activeExerciseIndex] || activeConcept.exercises[0];
  ...
  const currentCode = useMemo(() => {
    ...
    if (code === activeExercise.codeTemplate && !code.includes("// Sample usage")) {
      return getFullExerciseTemplate(activeExercise);
    }
    return code;
  }, [userCodes, activeExercise]);

  const prevExercise = activeExerciseIndex > 0 ? activeConcept.exercises[activeExerciseIndex - 1] : null;
  ...
  const prevCode = useMemo(() => {
    ...
    if (code === prevExercise.codeTemplate && !code.includes("// Sample usage")) {
      return getFullExerciseTemplate(prevExercise);
    }
    return code;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const allPassed = evaluation.results && evaluation.results.length > 0 && evaluation.results.every(res => res.passed);
        if (allPassed) {
          const updatedProgress = { ...solvedExercises, [activeExercise.id]: true };
  ...
  const handleResetCode = () => {
    if (confirm("Are you sure you want to reset this exercise to its default starter code? Any changes will be overwritten.")) {
      const updated = { ...userCodes, [activeExercise.id]: getFullExerciseTemplate(activeExercise) };
      saveCodesToStorage(updated);
      setTestResults(null);
    }
  };
  ...
  // Calculate stats
  const totalExercisesCount = CONCEPTS.reduce((sum, c) => sum + c.exercises.length, 0);
  const totalSolvedCount = Object.keys(solvedExercises).filter(id => solvedExercises[id]).length;

  const currentConceptSolvedCount = activeConcept.exercises.filter(e => solvedExercises[e.id]).length;
  const isConceptMastered = currentConceptSolvedCount === activeConcept.exercises.length;
  ...
  const sortedKnowledgeMapTopics = useMemo(() => {
    return [...KNOWLEDGE_MAP_TOPICS].sort((a, b) => {
      const idxA = CONCEPTS.findIndex(c => c.exercises.some(ex => ex.id === a.relatedExerciseId));
      const idxB = CONCEPTS.findIndex(c => c.exercises.some(ex => ex.id === b.relatedExerciseId));
  ...
  const sortedKnowledgeMapCategories = useMemo(() => {
    return [...KNOWLEDGE_MAP_CATEGORIES].sort((a, b) => {
      const firstTopicIndexA = sortedKnowledgeMapTopics.findIndex(t => t.categoryId === a.id);
      const firstTopicIndexB = sortedKnowledgeMapTopics.findIndex(t => t.categoryId === b.id);
            ...
            <div className="bg-zinc-900 border-b border-zinc-850 px-6 py-2.5 flex items-center justify-between shrink-0 shadow-sm text-white">
              <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:block">
                  <span className="text-[9px] font-mono text-zinc-400 uppercase font-semibold">Active Concept</span>
                  <p className="text-xs font-bold font-sans text-zinc-200 leading-none">{activeConcept.title}</p>
                </div>
              </div>
                  ...
                  {activeConcept.exercises.map((ex, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-7 w-7 rounded-md text-[11px] font-mono font-bold flex items-center justify-center transition-all cursor-pointer ${isCur
                ...
                <div className="p-4 flex-1 flex flex-col justify-between overflow-y-auto min-h-0">
                  ...
                  {currentExerciseStatus && (
                    <div className="mt-8 bg-zinc-900 text-white rounded-xl p-4 flex items-center gap-3 shadow-md border border-zinc-800">
                      ...
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[#F7DF1E]">Repetition Completed!</h4>
                        <p className="text-xs text-zinc-300 font-sans">
                          You solved this repetition. Switch to the next to keep building muscle!
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {isConceptMastered && (
                    <div className="mt-4 bg-emerald-950 text-white rounded-xl p-4.5 border border-emerald-800 shadow-lg animate-fade-in space-y-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-emerald-300">🎓 Concept fully Mastered!</h4>
                      </div>
                      <p className="text-[11px] text-zinc-200 leading-relaxed font-sans">
                        Amazing job! You successfully completed all {activeConcept.exercises.length} repetitions for <strong>{activeConcept.title}</strong>.
                        Head over to the <strong>Concept Guide</strong> tab on the left to review your interactive, beginner-friendly Concept Recap!
                      </p>
                      <button
