...
const CATEGORIES = [
  {
    ...
    icon: Layers,
    conceptIds: ["map-callbacks", "filter-callbacks", "reduce-callbacks"],
  },
  {
    id: "functional-oop",
    ...
    <section className="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-zinc-200 bg-white flex flex-col pt-5 px-4 overflow-y-auto shrink-0 max-h-[300px] lg:max-h-none">
      <div className="space-y-6 pb-6">
        {CATEGORIES.map((category) => {
          ...
          // Filter concepts that belong to this category and retain their relative source order
          const categoryConcepts = concepts.filter((c) =>
            category.conceptIds.includes(c.id),
          );
