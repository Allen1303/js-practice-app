      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-8 flex flex-col gap-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 bg-white border border-zinc-200 p-4 rounded-xl shadow-sm justify-between">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {Object.keys(completedTopics).some(
            ...
            const completedInCat = knowledgeMapTopics.filter(
              (t) =>
                ...
                (completedTopics[t.id] ||
                  (t.relatedExerciseId &&
                    solvedExercises[t.relatedExerciseId])),
            ).length;
            const isCatFinished =
          ...
          {filteredTopics.length === 0 ? (
            ...
            filteredTopics.map((topic) => {
              ...
              const isChecked =
                ...
                (topic.relatedExerciseId &&
                  !!solvedExercises[topic.relatedExerciseId]);
              const isExpanded = expandedTopicId === topic.id;
              const hasMatchingExercise =
                topic.relatedExerciseId &&
                concepts.some((c) =>
                  c.exercises.some((ex) => ex.id === topic.relatedExerciseId),
                );
                  ...
                  {!isExpanded && (
                    ...
                    >
                      ...
                      <span className="text-amber-600 font-mono text-[10px] ml-1.5 hover:underline font-bold">
                        {hasMatchingExercise
                          ? "Read details & explore snippet \u2192"
                          : "Review Deep-Dive Concept \u2192"}
                      </span>
                    </div>
                  )}
                        ...
                        <div className="pt-4 border-t border-zinc-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-zinc-505">
                              {hasMatchingExercise
                                ...
                                : "This is a theoretical study topic. Study the concept and takeaways above, then mark it complete manually!"}
                            </span>
                          </div>
                          ...
                          {hasMatchingExercise ? (
                            ...
                            <button
                              ...
                              className={`py-2.5 px-5 font-mono text-xs font-bold rounded-xl flex items-center gap-2 shadow-md hover:shadow cursor-pointer transition-all active:scale-[0.98] border ${
                                isChecked
                                  ? "bg-zinc-100 border-zinc-200 text-zinc-505 hover:bg-zinc-200"
                                  : "bg-[#F7DF1E] border-zinc-950 text-zinc-950 hover:bg-[#ffe63b]"
                              }`}
                            >
                              {isChecked
                                ? "✔ Completed Topic (Manual)"
