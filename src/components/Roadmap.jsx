...
import { Search, HelpCircle, ChevronRight, Check, CheckCircle, Sparkles, ArrowRight, BookMarked, Lock } from "lucide-react";
...
export function Roadmap({
  ...
  const filteredTopics = useMemo(() => {
    return knowledgeMapTopics.filter(topic => {
      const matchesCategory = selectedCategoryFilter === "all" || topic.categoryId === selectedCategoryFilter;
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-8 flex flex-col gap-8 pb-20">
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-6 md:p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="space-y-2 max-w-2xl">
            ...
            <p className="text-sm text-zinc-600 leading-relaxed">
              Track your mastery through the official 12-chapter curriculum structure modeled after <strong className="text-zinc-900 font-semibold">LearnJS.online</strong>. Tick topics off when you are confident, and jump straight into the Interactive Sandbox to practice coding loops, strings, and lesson exercises!
            </p>
          </div>
          ...
          <div className="bg-zinc-50 border border-zinc-200/80 p-5 rounded-2xl flex items-center justify-center gap-5 shrink-0 max-w-sm w-full md:w-auto shadow-inner select-none">
            <div className="text-center space-y-1">
              <span className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-widest block font-bold">YOUR COVERAGE</span>
              <div className="flex items-baseline justify-center gap-1.5">
                <span className="text-2xl font-mono font-black text-zinc-900">{completedTopicsCount}</span>
                <span className="text-xs text-zinc-400 font-mono">/</span>
                <span className="text-sm text-zinc-505 font-mono">{totalTopicsCount}</span>
              </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 bg-white border border-zinc-200 p-4 rounded-xl shadow-sm justify-between">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-[11px] font-mono text-zinc-455 uppercase hidden lg:inline font-bold">Filters Active:</span>
            ...
            {Object.keys(completedTopics).some(key => completedTopics[key]) && (
              <button
        ...
        <div className="flex flex-wrap gap-2 pb-1 border-b border-zinc-200">
          <button
            ...
            className={`py-2 px-4 rounded-xl text-xs font-bold font-mono transition-all border cursor-pointer ${selectedCategoryFilter === "all"
          ...
          {knowledgeMapCategories.map((cat) => {
            const countOfCat = knowledgeMapTopics.filter(t => t.categoryId === cat.id).length;
            const completedInCat = knowledgeMapTopics.filter(t => t.categoryId === cat.id && (completedTopics[t.id] || solvedExercises[t.relatedExerciseId])).length;
            const isCatFinished = completedInCat === countOfCat && countOfCat > 0;

            return (
              <button
                key={cat.id}
                onClick={() => setSelectedCategoryFilter(cat.id)}
                className={`py-2 px-3.5 rounded-xl text-xs font-bold font-mono transition-all border flex items-center gap-1.5 cursor-pointer ${selectedCategoryFilter === cat.id
                    ? "bg-zinc-900 border-zinc-950 text-white shadow-sm"
              ...
              >
                ...
                <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${isCatFinished ? "bg-[#F7DF1E]/20 text-zinc-800 font-bold" : "bg-zinc-100 text-zinc-505"
                  }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredTopics.length === 0 ? (
            <div className="col-span-full bg-white border border-dashed border-zinc-300 p-12 rounded-2xl text-center space-y-2">
              ...
              <h3 className="text-base font-bold text-zinc-900">No syllabus topics matching "{searchQuery}"</h3>
              <p className="text-xs text-zinc-505 max-w-sm mx-auto font-normal">
                Try entering different Javascript search words, or clear the searches and verify the active Chapter headings.
              </p>
              <button
            ...
            filteredTopics.map((topic) => {
              const category = knowledgeMapCategories.find(c => c.id === topic.categoryId);
              const isChecked = !!completedTopics[topic.id] || !!solvedExercises[topic.relatedExerciseId];
              ...
              return (
                <motion.div
                  layout="position"
                  key={topic.id}
                  className={`bg-white border rounded-2xl shadow-sm overflow-hidden transition-all duration-305 ${isExpanded
                ...
                >
                  ...
                  <div
                    ...
                    className={`p-5 flex items-start gap-4 justify-between select-none ${isUnlocked ? "cursor-pointer group" : "cursor-not-allowed"
                      }`}
                  >
                    <div className="flex items-start gap-3.5 flex-1 select-none">
                      ...
                      <button
                        ...
                        title={!isUnlocked ? "This lesson is locked" : isChecked ? "Mark topic as incomplete" : "Mark topic as mastered"}
                      ...
                      <div className="space-y-1.5 select-none text-left">
                        <span className={`inline-block text-[9px] font-mono font-bold tracking-wider uppercase px-2 py-0.5 rounded-md ${category ? category.lightColor : "bg-zinc-100 text-zinc-505"
                          }`}>
                          {category ? category.title.split(". ")[1] : "Syllabus Group"}
                        </span>
                        ...
                        <h3 className="text-sm font-extrabold text-zinc-900 tracking-tight group-hover:text-zinc-950 select-none flex items-center gap-1.5">
                          {!isUnlocked && <Lock className="h-3 w-3 text-zinc-400 shrink-0" />}
                          {topic.title}
                        </h3>
                      </div>
                    </div>
                    ...
                    <div className="h-8 w-8 rounded-lg bg-zinc-50 flex items-center justify-center border border-zinc-200 text-zinc-400 group-hover:border-zinc-350 shrink-0 mt-0.5">
                      {!isUnlocked ? (
                        ...
                        <ChevronRight className={`h-4.5 w-4.5 text-zinc-650 transition-transform duration-200 ${isExpanded ? "rotate-90 text-zinc-900" : ""
                          }`} />
                      )}
                    </div>
                  </div>
                  ...
                  {!isExpanded && (
                    ...
                    >
                      {topic.summary} <span className="text-amber-600 font-mono text-[10px] ml-1.5 hover:underline font-bold">Read details & explore snippet &rarr;</span>
                    </div>
                  )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3.5">
                            ...
                            <div className="space-y-2 bg-yellow-50/20 border border-amber-100 rounded-xl p-4">
                              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-amber-800 flex items-center gap-1 font-bold">
                                <Sparkles className="h-3.5 w-3.5 text-amber-600" /> Key Takeaways Checklist
                              </span>
                              <ul className="space-y-2 text-left">
                                {topic.takeaways.map((takeaway, tIdx) => (
                                  <li key={tIdx} className="text-xs text-zinc-700 flex items-start gap-1.5 font-normal">
                                    <span className="text-[#a38b00] font-bold shrink-0 text-sm mt-[-2px]">&bull;</span>
                                    <span>{takeaway}</span>
                                  </li>
                                ))}
                              </ul>
                          ...
                          <div className="flex flex-col border border-[#181a1f] rounded-xl overflow-hidden bg-[#282c34] shadow-sm">
                            <div className="flex items-center justify-between px-4 py-2 bg-[#21252b] border-b border-[#181a1f] shrink-0 select-none">
                              <span className="text-[10px] font-mono font-bold tracking-wider text-[#abb2bf] uppercase">Interactive Syntax Sandbox template</span>
                              <span className="text-[10px] font-mono text-[#5c6370]">VANILLA JS</span>
                            </div>
                            <pre
                              className="p-4 flex-1 overflow-x-auto text-[11px] font-mono text-[#abb2bf] bg-[#282c34] leading-relaxed select-all"
                              dangerouslySetInnerHTML={{ __html: highlightJS(topic.codeSnippet, true) }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-zinc-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-zinc-505">
                              Ready to build muscle memory with real code assertions? See sandbox exercises!
                            </span>
                          </div>

                          <button
                            onClick={() => jumpToExercise(topic.relatedExerciseId)}
                          ...
                          >
                            Test This in the Sandbox <ArrowRight className="h-4 w-4 text-[#F7DF1E]" />
                          </button>
                        </div>
